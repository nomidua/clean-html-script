...
(function() {
    ...
    window.cleanHTMLContent = function() {
        ...
        // 20.2. Добавляем пробел после знаков препинания (кроме ; и чисел с запятой)
// Защищаем ссылки
var links = [];
html = html.replace(/<a[^>]*>.*?<\/a>/gi, function(match) {
    links.push(match);
    return '___LINK_' + (links.length - 1) + '___';
});

// Добавляем пробелы ТОЛЬКО после букв (исключаем цифры из второй группы)
html = html.replace(/([.,!?:])([а-яёА-ЯЁa-zA-Zа-щА-ЩЬьЮюЯяЇїІіЄєҐґ])/g, '$1 $2');

// Возвращаем ссылки
for (var i = 0; i < links.length; i++) {
    html = html.replace('___LINK_' + i + '___', links[i]);
}
